  ...
  do {
    ...
    const response = await fetch(baseUrl, {
      ...
      headers: {
        ...
        "X-Goog-FieldMask":
          "places.displayName,places.formattedAddress,places.location,places.rating,places.userRatingCount,places.priceLevel,places.types,places.id,places.websiteUri",
      },
    ...
    if (action === "findAllRestaurants") {
      ...
      if (!latitude || !longitude || !radiusMiles) {
        return new Response(
          JSON.stringify({
            error:
              "findAllRestaurants requires `latitude`, `longitude`, and `radius` (in miles) query params",
          }),
          {
            status: 400,
            headers: {
      ...
      if (query) {
        targetUrl = `https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=${encodeURIComponent(
          query
        )}&inputtype=textquery&key=${apiKey}`;
      } else if (location && radius) {
        targetUrl = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${encodeURIComponent(
          location
        ...
        return new Response(
          JSON.stringify({
            error:
              "placeSearch requires `query` or (`location` and `radius`) query params",
          }),
          {
            status: 400,
            headers: {
      ...
      targetUrl = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${encodeURIComponent(
        place_id
      )}&key=${apiKey}`;
      ...
      targetUrl = `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(
        input
      )}&key=${apiKey}`;
    } else {
      return new Response(
